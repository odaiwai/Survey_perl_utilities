...
if ($do_rms) {
    ...
    foreach my $TAK (@TAKs) {
        ...
        my @directions = array_from_query($db, "Select distinct direction from [traffic_counts] where (traffic_asset_key = $TAK)", $verbose);
        foreach my $direction (@directions) {
            ...
            foreach my $year (2015..2017) {
                ...
                my $counts = sanitise_values (@counts);
                dbdo ($db, "insert or replace into [Y$year\_surveys] (anode, bnode, UUID_dir, distance_from_site, daily, am, ip, pm, ev) Values ($possible_a, $possible_b, \"$TAK\_$direction\", $shortest_distance $counts)", $verbose);
            }
        }
        dbdo($db, "COMMIT", $verbose);
    }
 #   dbdo($db, "COMMIT", $verbose);
    foreach my $year (2015..2017) {
        $result = `sqlite3 traffic_counts.sqlite -header -csv "select * from [Y$year\_surveys] order by UUID_dir;" > y$year\_surveys.csv`;
    }
}
...
sub make_tables {
    my %tables = (
        ...
        "Y2015_surveys"=> "anode Integer, bnode Integer, UUID_dir Text Primary Key, distance_from_site Real, daily REAL, am REAL, ip REAL, pm REAL, ev REAL",
        "Y2016_surveys"=> "anode Integer, bnode Integer, UUID_dir Text Primary Key, distance_from_site Real, daily REAL, am REAL, ip REAL, pm REAL, ev REAL",
        "Y2017_surveys"=> "anode Integer, bnode Integer, UUID_dir Text Primary Key, distance_from_site Real, daily REAL, am REAL, ip REAL, pm REAL, ev REAL");
